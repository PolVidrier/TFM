...
def BDT_Training(mcfile,mctree,cutsmc,datafile,datatree,cutsdata,variables,name):
    ...
    plt.ylabel(None)
    plt.grid(False)
    plt.savefig("BDT/plots/BDT/Feature_Importance.png", dpi=300)
    plt.close()
    ...
    plt.savefig("BDT/plots/BDT/Predictions.png")
    plt.close()
    ...
    plt.savefig("BDT/plots/BDT/ROC.png")
    plt.close()
    ...
    plt.title("ROC")
    plt.legend(loc="lower right")
    plt.savefig("BDT/plots/BDT/ROC_corner.png")
    plt.close()
    ...
    plt.title("ROC")
    plt.legend(loc="lower right")
    plt.savefig("BDT/plots/BDT/ROC_error.png")
    plt.close()
    ...
    plt.savefig("BDT/plots/BDT/FoM.png")
    plt.close()
    ...
    fom=[]
    errorFoM=[]
    cuts=np.linspace(0.8,0.9999,1000)
    for cut in cuts:
        S=0
        B=0
        for varsig in pred_probsignal:
    ...
    plt.title("FoM")
    plt.legend(loc="lower center")
    plt.savefig("BDT/plots/BDT/FoM_peak.png")
    plt.close()
...
mctree="DecayTree"
cutsmc="Bu_BKGCAT==0"
cutsmcplus="(Bu_BKGCAT==0) & (acos((ep_PX*em_PX+ep_PY*em_PY+ep_PZ*em_PZ)/(ep_P*em_P))>0.0005) & (acos((ep_PX*Kp_PX+ep_PY*Kp_PY+ep_PZ*Kp_PZ)/(ep_P*Kp_P))>0.0005) & (acos((Kp_PX*em_PX+Kp_PY*em_PY+Kp_PZ*em_PZ)/(Kp_P*em_P))>0.0005) & (ep_PT>500) & (em_PT>500)"
