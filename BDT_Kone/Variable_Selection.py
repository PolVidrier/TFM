...
for varmc in feature_listmc:
    for var in feature_list: 
        if var==varmc:
            if var in ["ep_CLUSTERMATCH","Bu_M","Jpsi_M","Jpsi_PZ","Bu_MIN_PT","Jpsi_PT","Bu_BPVIP","em_ETA","ep_ETA","Jpsi_BPVIPCHI2","Bu_BPVIPCHI2","Kp_PT"]:
                dictionary[var]=[min([min(vararray[var]),min(vararraymc[var])]),max([max(vararray[var]),max(vararraymc[var])])]

with open("BDT_Kone/data/variableselection.yaml","w") as file:
