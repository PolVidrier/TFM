...
def BDT_Training(mcfile,mctree,cutsmc,datafile,datatree,cutsdata,variables,name):
    ...
    #xgb_classifier = xgb.XGBClassifier(gamma=1,max_depth=5,min_child_weight=2) #optimized
    ...
    plt.figure()
    plt.plot(fpr, tpr, color="darkorange",lw=2, label="XGB-AUC_test= %0.10f" % roc_auc,)
    plt.plot(fprtrain, tprtrain, color="darkgreen",lw=2, label="XGB-AUC_train= %0.10f" % roc_auctrain,)
