...
data=uproot.open(r"/eos/lhcb/user/p/pvidrier/roots/data_Ktwo_with_cuts.root")["DecayTree"]
weights=uproot.open(r"BDT_Ktwo/weights/weights.root")["RooTreeDataStore_ds_data_ds_data"]
...
for var in dictionary:
        if var!="sBu_BPVIPCHI2":
            if type(dictionary[var][0])!=bool:
                ...
                plt.savefig("BDT_Ktwo/plots/Weighted_plots/{}.png".format(var))
                plt.close()
            else:
